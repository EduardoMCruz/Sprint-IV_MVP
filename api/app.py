...
paciente_tag = Tag(
    name="Paciente",
    description="Adição, visualização, remoção e predição dos pacientes que realizaram o teste do covid-19",
)
...
def get_pacientes():
    ...
    Args:
         (str): código do paciente que realizou o teste
...
def predict(form: PacienteSchema):
    ...
    Args:
        ...
        Glóbulos Brancos (int): número de Glóbulos Brancos no sangue
        Bbasofilos (int): concentração de basofilos
        Escória Nitrogenada (int): Funsão Renal (escoria nitrogenada)
        Proteina_c (int): espessura proteina_c
        Glóbulos Vermelhos (int): Glóbulos Vermelhos no sangue
    ...
    # Carregando modelo
    ml_path = "ml_model/covid-19_knn.pkl"
    scaler_path = joblib.load("ml_model/scaler.joblib")
    modelo = Model.carrega_modelo(ml_path, scaler_path)

    print("==============================================")
    print(form.idade)
    print(form.leucocitos)
    print(form.basofilos)
    print(form.creatinina)
    print(form.proteina_c)
    print(form.hemoglobina)
    print("==============================================")
    paciente = Paciente(
        idade=form.idade,
    ...
    logger.debug(f"Adicionando paciente de idade: '{paciente.idade}'")
